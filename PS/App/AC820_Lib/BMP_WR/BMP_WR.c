...
void bmp_write(char * name, char *head_buf, char *data_buf)
{
    ...
    iPixelAddr = (Yimage-1)*Ximage*2 ;
    for(y = 0; y < Yimage ; y++)
    {
        for(x = 0; x < Ximage; x++)
        {
        	Write_line_buf[x*3 + 0] = (data_buf[x*2 + iPixelAddr + 0] & 0x1F) << 3;
			Write_line_buf[x*3 + 1] = ((data_buf[x*2 + iPixelAddr + 1] & 0x07)<<5)
									  |((data_buf[x*2 + iPixelAddr + 0]& 0xE0)>> 3);
			Write_line_buf[x*3 + 2] = data_buf[x*2 + iPixelAddr + 1] & 0xF8;
        }
